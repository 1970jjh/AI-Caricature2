...
export const generatePostItCaricature = async (drawingBase64: string): Promise<string> => {
  try {
    ...
    const prompt = `이미지에서 포스트잇 부분을 추출하여 화려하고 생동감 있는 캐리커쳐 스타일로 변환하고, 세련된 액자에 담아주세요.
...
6. 액자 스타일 (매우 중요):
   - 현대적이고 미니멀한 슬림 액자
   - 액자 테두리는 최대한 얇게 (전체 이미지의 5% 이하)
   - 화이트 또는 골드 색상의 깔끔한 직선 프레임
   - 고풍스러운 장식이나 조각 없이 심플하게
7. 손그림/캐리커쳐가 전체 이미지의 85% 이상을 차지하도록 해주세요
8. 최종 이미지는 액자 프레임까지만 보이도록 크롭해주세요
9. 그림이 주인공이고, 액자는 그림을 돋보이게 하는 보조 역할만 해주세요`;
